    ...
    private fun createAndSendLog(
        ...
        if (!value.toString().contains("index split is empty") && !value.toString()
            ...
            try {
                ...
                val log = nativeLog.LogBuilder().logType(level).category(LogCategory.OTA_LIFE_CYCLE)
                    .eventName(eventName).value(jsonData)
    ...
    override fun trackException(
        ...
        if (category == "lifecycle") {
            ...
            try {
                val jsonData = JSONObject(eventData).toString()
                val log =
                    nativeLog.LogBuilder().logType("error").category(LogCategory.OTA_LIFE_CYCLE)
